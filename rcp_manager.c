...
rcp_manager* rcp_manager_create(void* user)
{
    ...
    if (manager != NULL)
    {
        ...
        {
            RCP_DEBUG("!! manager without user");
        }
...
void rcp_manager_clear(rcp_manager* manager)
{
    if (manager)
    {
        ...
        while (pe)
        {
            ...
            RCP_DEBUG("+++ removed parameter list entry: %p\n", pe);
            RCP_FREE(pe);
...
bool rcp_manager_update_parameter(rcp_manager* manager, rcp_parameter* parameter, bool is_server)
{
    ...
    {
        RCP_DEBUG("update cached parameter - server: %d\n", is_server);
    ...
    else if (!is_server)
    {
        ...
        RCP_DEBUG("could not add parameter to cache %d\n", rcp_parameter_get_id(parameter));
    }
    else
    {
        RCP_DEBUG("parameter not in chache - server can not add parameter!\n");
    }
...
bool rcp_manager_remove_parameter_id(rcp_manager* manager, int16_t parameter_id, bool is_server)
{
    ...
    while (entry)
    {
        ...
        {
            RCP_DEBUG("remove dirty parameter: %d\n", parameter_id);
