...
void rcp_client_free(rcp_client* client)
{
    if (client)
    {
        ...
        {
            RCP_DEBUG("+++ client application id: %p\n", client->applicationId);
...
void rcp_client_set_id(rcp_client* client, const char* id)
{
    ...
    if (id != NULL)
    {
        ...
        if (str_len > 0)
        {
            ...
            {
                RCP_DEBUG("client: could not calloc for applicationid\n", client->applicationId);
            }
        }
    }
}
...
static inline void _do_command_info(rcp_client* client, rcp_packet* packet)
{
    ...
    {
        RCP_DEBUG("no data, no transporter! did not query version from server\n");
    }
}
           ...
           && size > 0)
    {
        ...
        if (data && packet)
        {
            ...
            case COMMAND_INITIALIZE:
                // no init on client
                RCP_DEBUG("ignore command 'initialize' on client!\n");
                break;

            case COMMAND_DISCOVER:
                // no discovery on client
                RCP_DEBUG("ignore command 'discover' on client!\n");
               break;
            ...
            case COMMAND_UPDATEVALUE:
            {
                    ...
                    if (parameter)
                    {
                                    ...
                                    ))
                        {
                            ...
                            if (cached_parameter)
                            {
                                ...
                                {
                                    RCP_DEBUG("client: update: bang parameter type missmatch\n");
                                }
                            }
                            else
                            {
                ...
                else
                {
                    ...
                    RCP_DEBUG("client does not accept parameters! - %d\n", id);
                }
#endif
                break;
            }
