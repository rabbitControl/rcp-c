...
static char set_buffer_size(rcp_sppp* pp, size_t size)
{
    ...
    // try to realloc
    RCP_DEBUG("sppp try to realloc: %lu\n", size);
    ...
    if (new_p == NULL)
    {
        RCP_DEBUG("sppp could not realloc buffer: %lu\n", size);
        return 0;
    }

    RCP_DEBUG("*** sppp buffer: %p\n", new_p);
...
static char memcpy_data_to_buffer(rcp_sppp* pp, void* data, size_t size)
{
    ...
    RCP_DEBUG("adding to buffer: %lu\n", size);

    memcpy(pp->buffer + pp->current_idx, data, size);
    pp->current_idx += size;

    RCP_DEBUG("buffer index: %lu\n", pp->current_idx);
...
void rcp_sppp_data(rcp_sppp* pp, char* data, size_t size)
{
	...
	RCP_DEBUG("rcp_sppp_push_data: %d - %d\n", size, data[0]);
    ...
    while (size > 0)
    {
        ...
        RCP_DEBUG("packet_size: %d\n", pp->packet_size);
        RCP_DEBUG("current_idx: %d\n", pp->current_idx);
        ...
        else
        {
            ...
            if (size_to_packet <= size)
            {
                if (pp->current_idx == 0)
                {
                    ...
                    {
                        RCP_DEBUG("sppp - dropping invalid packet\n");
                    }
                }
                else
                {
                    ...
                    if (RCP_CHECKFLAG(pp->flags, SPPP_PACKET_INVALID))
                    {
                        RCP_DEBUG("sppp - dropping data -> invalid packet\n");
                    }
                }
            ...
            else
            {
                        ...
                        !memcpy_data_to_buffer(pp, data, size))
                {
                    ...
                    RCP_DEBUG("packet invalid or could not copy data to buffer\n");
